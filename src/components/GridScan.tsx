...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  const [modelsReady, setModelsReady] = useState(false);
    ...
    const loadedModelsPathRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    (u.uLinesColor.value as any).copy(srgbColor(linesColor));
    (u.uScanColor.value as any).copy(srgbColor(scanColor));
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      if (loadedModelsPathRef.current === modelsPath && modelsReady && faceapiRef.current) return;
      let canceled = false;
      const load = async () => {
        try {
          ...
          loadedModelsPathRef.current = modelsPath;
          if (!canceled) setModelsReady(true);
        } catch {
          loadedModelsPathRef.current = null;
    ...
    }, [enableWebcam, modelsPath, modelsReady]);
